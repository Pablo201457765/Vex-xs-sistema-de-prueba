...
const SETTINGS_KEY = 'vexs_u4_6';
let SETTINGS = JSON.parse(localStorage.getItem(SETTINGS_KEY) || '{}');
if(!SETTINGS.iconSize) SETTINGS.iconSize = 64;
if(!SETTINGS.iconShape) SETTINGS.iconShape = 'circle';
...
function buildHome(){
  ...
  appsRow.innerHTML = '';
  APPS.forEach(a=>{
    const tile = document.createElement('button'); tile.className='app-tile';
    tile.innerHTML = `<img class="app-icon" src="${a.icon}" alt="${a.name}" onerror="imgFallback(this)"><div class="app-label">${a.name}</div>`;
    tile.addEventListener('click', ()=> openApp(a.id));
    appsRow.appendChild(tile);
  });
  applyIconSettings();
}
...
function imgFallback(img){
  const placeholder = 'data:image/svg+xml;utf8,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120"><rect rx="14" width="100%" height="100%" fill="#444"/><text x="50%" y="55%" font-size="48" text-anchor="middle" fill="#ffcc00">!</text></svg>`);
  img.src = placeholder;
  const srcAttr = img.getAttribute('src') || 'icon';
  showToast(`⚠️ Falta ${srcAttr} en la raíz`, 20000);
}

function showToast(text, duration=20000){
...
function bootSeq(){ setTimeout(()=>{ document.getElementById('boot').classList.add('hidden'); document.getElementById('lock').classList.remove('hidden'); }, 1000); }

function enableGestures(){
  const lock = document.getElementById('lock'); let sy=0;
  ...
  const appWin = document.getElementById('appWindow'); let as=0;
...
function openConfig(){
  ...
  document.querySelectorAll('.opt').forEach(el=> el.addEventListener('click', ()=>{
    ...
    if(key==='about') showSub('<h2>Sobre el teléfono</h2><p>Vex XS UltimatePlus 4.6</p>');
...
function showStyle(){
  ...
  content.innerHTML = `<h2>Altura y estilo</h2>
    <label>Tamaño: <input id="sizeRange" type="range" min="40" max="140" value="${SETTINGS.iconSize}"></label>
...
function applyStyle(){ const v=parseInt(document.getElementById('sizeRange').value,10); const s=document.getElementById('shapeSel').value; SETTINGS.iconSize=v; SETTINGS.iconShape=s; saveSettings(); applyIconSettings(); showSub('<h2>Estilo aplicado</h2><button onclick="closeApp()">Atrás</button>'); }

function applyIconSettings(){ document.documentElement.style.setProperty('--icon-size', SETTINGS.iconSize + 'px'); let radius='50%'; if(SETTINGS.iconShape==='square') radius='6px'; if(SETTINGS.iconShape==='round') radius='20%'; document.documentElement.style.setProperty('--icon-radius', radius); if(SETTINGS.iconSize>120) showToast('⚠️ Tamaño muy grande — reduce para ver todas las apps',4000); }
...
function openBrowser(){ showSub('<h2>Navegador (demo)</h2><p>Funcionalidad completa en futuras versiones.</p><button onclick="closeApp()">Atrás</button>'); }

// Init
document.addEventListener('DOMContentLoaded', ()=>{ applyBackgrounds(); buildHome(); enableGestures(); bootSeq(); document.getElementById('dock').style.display='flex'; });